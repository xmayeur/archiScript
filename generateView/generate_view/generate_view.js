...
function generate_view(param) {
    /*
        Param is an object that may contain the following properties:
            ...
            OnlySelectedRelationships (optional) Only represent relationships from the selected concept
            								Defaults to true
    ...
    */
    ...
    else if (G_debug) G_verbose = true;
	...
	debug("     - OnlySelectedRelationships = " + param.OnlySelectedRelationships);
    ...
    if (param.OnlySelectedRelationships === undefined)
    ...
    if (param.action === undefined || param.action === TYPE_ACTION.ONE_SINGLE_VIEW) {
        ...
        view = _generate_view(selectedElements, param.nestedRelationships, param.graphDirection, param.graphAlign, param.hSep, param.vSep, param.algorithm, param.graphDepth, param.viewName, param.OnlySelectedRelationships);
    } else {
        selectedElements.forEach(function (concept) {
            view = _generate_view(concept, param.nestedRelationships, param.graphDirection, param.graphAlign, param.hSep, param.vSep, param.algorithm, param.graphDepth, param.OnlySelectedRelationships);
        });
    }
...
function _generate_view(elem, nestedRelationships, graphDirection, graphAlign, hSep, vSep, algorithm, graphDepth, viewName, OnlySelectedRelationships) {
    var counter = startCounter();
        ...
        for (id in elements) {
            ...
            $(sourceElement).outRels().each(function (rel) {
                ...
                if ($(targetElement).is("element")) {
                    ...
                    var trouve = false;
                    for (e in elements) {
                        if (elements[e].id === targetElement.id) {
                            trouve = true;
                            break;
                        }
                    }
            ...
            $(sourceElement).inRels().each(function (rel) {
                var targetElement = $(rel).targetEnds().first();
                if ($(targetElement).is("element")) {
                    // we have to manually loop on the array as the proxy object is different from the one in the array
                    // so the indexOf() function always fails
...
/// <- XMA
    ...
    if (!OnlySelectedRelationships) {
        ...
        for (id in elements) {
            ...
            $(sourceElement).inRels().each(function (rel) {
                var targetElement = $(rel).sourceEnds().first();
                if ((viewObjects[targetElement.id] !== undefined) && (nestedRelationships.indexOf(rel.type) == -1)) {
                    // if the target element is in the view and that the objects are not nested
                    if (addedRelationships.indexOf(rel.id) === -1) {
                        debug("     - connecting " + sourceElement + " to " + targetElement);
                        addedRelationships.push(rel.id);
                        var connection = view.add(rel, viewObjects[targetElement.id], viewObjects[sourceElement.id]);
                        // TODO: if the graph contains control points for edges, use them as connection corners
                    }
                }
            });
